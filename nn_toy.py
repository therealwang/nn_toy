...
class Network:
    def __init__(self, layers, cost = dQuadCost):
        ...
        for i in range(len(layers)-1):
            self.weights.append(np.random.randn(layers[i+1],layers[i])/np.sqrt(layers[i]))
    ...
    def train(self, batch, eta, lmb):
        ...
        '''
            for i in range(len(errors)):
                self.biases[i] -= self.alpha * errors[i]
                self.weights[i] = self.weights[i] - self.alpha * (np.matmul(errors[i] , forwardoutputs[i].transpose() ))
        '''
